...
def graph(timeMin, timeMax, numNodes, seed):
    ...
    :param numNodes: number of nodes for which the graph is produced
    :param seed: the random number seed for np.random.seed

    :return: mean path length and cluster coef vs time step 

    """
    
    # this assumes that numNodes is a perfect square
    dim = int(np.sqrt(numNodes))
    ...
    # this will contain all information about the network through the walk
    # instantiates to add all agent_IDs as keys without values 
    # this will act as an adjacency list 
    # in order for networkx to accept this as a graph though it needs to become
    # a dict of dicts
    network_dict = dict(zip(agent_IDs, [None]*len(agent_IDs))) 
